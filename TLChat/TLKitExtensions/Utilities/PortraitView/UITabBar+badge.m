...
- (void)showBadgeOnItemIndex:(int)index{
  ...
  CGFloat y = ceilf(0.01 * tabFrame.size.height);
  badgeView.frame = CGRectMake(x, y, 10, 10);//圆形大小为10
  [self addSubview:badgeView];
}

- (BOOL)hasRedDotBadgeOnItemIndex:(int)index {
 
    if ([self viewWithTag:888 + index]) {
        return YES;
    }
    return NO;
}

- (BOOL)hasBadgeValueOnItemIndex:(int)index {

    UILabel * label = (UILabel*)[self viewWithTag: (1000 + index)];
    if (label) {
        return [label.text integerValue] > 0;
    }
